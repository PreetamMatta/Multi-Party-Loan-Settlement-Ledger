...
class TestValidateEventFields:
    ...
    def test_compensating_entry_for_non_financial_event_does_not_require_amount(self, make_event):
        """
        A COMPENSATING_ENTRY that reverses a non-financial event (like RATE_CHANGE)
        inherits its `amount_property_currency = None`. The validator must not
        reject it for missing an amount, because the original legitimately had none.
        """
        evt = make_event(
            event_type=EventType.COMPENSATING_ENTRY,
            amount_property_currency=None,
            reverses_event_id=uuid.uuid4(),
            metadata={"original_event_type": EventType.INTERPERSONAL_RATE_CHANGE.value},
        )
        errors = validate_event_fields(evt)
        assert not any("amount_property_currency is required" in e for e in errors)

    def test_compensating_entry_for_financial_event_missing_amount_is_rejected(self, make_event):
        """
        A COMPENSATING_ENTRY that reverses a financial event must have an amount.
        """
        evt = make_event(
            event_type=EventType.COMPENSATING_ENTRY,
            amount_property_currency=None,
            reverses_event_id=uuid.uuid4(),
            metadata={"original_event_type": EventType.CONTRIBUTION.value},
        )
        errors = validate_event_fields(evt)
        assert any("amount_property_currency is required" in e for e in errors)

    def test_compensating_entry_with_invalid_parent_type_ignores_amount_check(self, make_event):
        """
        A COMPENSATING_ENTRY with garbage original_event_type ignores the check
        ...
        evt = make_event(
            ...
            amount_property_currency=None,
            reverses_event_id=uuid.uuid4(),
            metadata={"original_event_type": "GARBAGE_TYPE"},
        )
        errors = validate_event_fields(evt)
        assert not any("amount_property_currency is required" in e for e in errors)
...
class TestFinancialEffect:
    def test_get_financial_effect_unhandled_type(self, make_event):
        """
        If an unrecognized event type is somehow passed, the router must raise
        a ValueError rather than silently returning an empty effect.
        """
        # Mutate the event_type directly to bypass Pydantic static instantiation validation
        evt = make_event()
        evt.event_type = "NON_EXISTENT_TYPE"

        with pytest.raises(ValueError, match="Unhandled event type: NON_EXISTENT_TYPE"):
            get_financial_effect(evt)
